...
void Player::keyPressed(int key){
    ...
    else if(key == 'c') { // clear all ingredients
       burger->eraseBurger();
    }
    else if(key == 'u') { // clear last ingredient
        burger->undoIngredient();
    }
}

double Player::getBurgerPrice(){
    return burger->getBurgerPrice(this->burger);
}
void Player::resetPrice(){
    burger->resetPrice();
}
